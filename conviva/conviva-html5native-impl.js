...
function Html5PlayerInterface (playerStateManager, videoElement) {
    ...
    _s._timeupdate /* : int */= 0;
    _s._lastTimeupdate /* : int */= 0;
    _s._currentTimeIsInvalid /* : bool */= false;
    ...
    this._registerVideoEventListeners = function () {
        ...
        _s._addEventListener("playing", function () {
            ...
            if (_s._videoElement.currentTime == 0){
            		_s._currentTimeIsInvalid = true;
            }
        });
        ...
        _s._addEventListener("timeupdate", function () {
			if (_s._currentTimeIsInvalid){
				_s._timeupdate ++;
				_s._log("timeupdate = " + _s._timeupdate);
				if (_s._playerStateManager.getPlayerState() !== Conviva.PlayerStateManager.PlayerState.PLAYING){
					_s._receivedHtml5Event("playing");
				}
			}
           _s._log("Ignored timeupdate event (player was playing)");
        });
        ...
        _s._addEventListener("seeking", function () {
            ...
            // When UC browser buffering
            if (_s._currentTimeIsInvalid) {
            		if (_s._playerStateManager.getPlayerState() !== Conviva.PlayerStateManager.PlayerState.BUFFERING){
...
//          currentPhtSpeed = ((_s._currentPosition - _s._previousPosition) / (now - _s._lastPollTime)) * 1000;
	    /**** In some cases,  the currentPosition might decrease.  we need to protect this situation ***/ 
            var position = _s._currentPosition - _s._previousPosition;
            if ( position < 0){
            		position = 0;
            }
            currentPhtSpeed = (position / (now - _s._lastPollTime)) * 1000;
    ...
    this._inferPlayerStateFromPosition = function () {
        ...
        if (numPhSpeedSamples >= Math.min(Html5PlayerInterface.PLAY_STATE_ON_MOVING_PHT_POLL_COUNT, Html5PlayerInterface.PAUSED_STATE_ON_CONSTANT_PHT_POLL_COUNT)) {
            ...
            if (currentConvivaState == Conviva.PlayerStateManager.PlayerState.PLAYING && numPhSpeedSamples >= Html5PlayerInterface.PAUSED_STATE_ON_CONSTANT_PHT_POLL_COUNT && avgSpeed == Html5PlayerInterface.PHT_PAUSED_EXPECTED_SPEED && !_s._currentTimeIsInvalid) {
                if (_s._videoElement.paused) {
                    _s._log("Adjusting Conviva player state to: PAUSED");
                    _s._updateConvivaPlayerState(Conviva.PlayerStateManager.PlayerState.PAUSED);
                    /*** “timeupdate” will be triggered is “Pause” button is clicked */
                    _s._timeupdate = _s._lastTimeupdate;
            ...
            if (_s._currentTimeIsInvalid) {
                    ...
                    if (_s._timeupdate > 1 && _s._timeupdate == _s._lastTimeupdate){
        					_s._log("Adjusting Conviva player state to: BUFFERING");
            				_s._updateConvivaPlayerState(Conviva.PlayerStateManager.PlayerState.BUFFERING);		
        				}
            			_s._lastTimeupdate = _s._timeupdate;
                }
        			return;
        		}
        }
	...
	this._resetTimeupdate = function () {
		_s._lastTimeupdate = 0;
		_s._timeupdate = 0;
	}
    // PlayerStateManager
